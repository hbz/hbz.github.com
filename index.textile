...
When we start working on an issue, we move it to the @working@ column. Ideally, every person should only work on one issue at a time. That way the @working@ column provides an overview of who is currently working on what. We include "references to the corresponding issue":https://guides.github.com/features/issues/#notifications in the commit messages. We _don't_ use "keywords for closing issues":https://help.github.com/articles/closing-issues-via-commit-messages/ in the commit messages, since the resolution decision does not happen during implementation, but during review (see below).
