...
When we start working on an issue, we move it to the @working@ column. Ideally, every person should only work on one issue at a time. That way the @working@ column provides an overview of who is currently working on what. For commits that resolve their corresponding issue, we include "keywords for closing issues":https://help.github.com/articles/closing-issues-via-commit-messages/.

h4(#dev-process-review). Review

When an issue is completed on the technical side, we push the corresponding commits to a feature branch that contains the corresponding issue number (and additional info for convenience), e.g. @111-ui@. If the issue is in a different repo than the branch, we prefix the repo name, e.g. @nwbib-111-ui@. If possible, we then deploy the changes from the feature branch to our staging system (by merging the feature branch into the master branch of the staging system, therefore simulating what will happen when we deploy to production). We then provide instructions on how to test the changes in the corresponding issue, and assign that to a dedicated team member (the product owner) for functional review. At the end of the functional review, the reviewer posts a @+1@ comment, moves the corresponding card to the @deploy@ column, and assigns it to the developer who submitted it for review.

When submitting an issue for review, we also open a corresponding pull request for the feature branch with a summary of the changes, a link to the corresponding issue using "keywords for closing issues":https://help.github.com/articles/closing-issues-via-commit-messages/ (this will result in Waffle grouping the issue and corresponding pull requests in a single card), and instructions on how to test the behaviour on staging. We leave the pull request self-assigned until the functional review concludes with a @+1@, i.e. the code is ready for review. If required, the developer then rebases the feature branch or merges the master branch to get the feature branch up to date with the master branch. We then unassigned the pull request to signal to the team members that it is ready for code review.

We use Travis CI for continuous integration. The CI is integrated into the GitHub review process: when a pull request is opened, Travis builds the resulting merged code and provides feedback right in the pull request (you can see the build status in the pull request preview, before submitting the pull request). For details, see "this post":http://blog.travis-ci.com/2012-09-04-pull-requests-just-got-even-more-awesome/. The developer who pushed the code should ensure the Travis build was successful before submitting the pull request for review (i.e. unassigning it). Changes to the code during code review are created in additional commits, which the developer pushes to the feature branch. They are added to the existing pull request automatically. See "details on pull requests":https://help.github.com/articles/using-pull-requests/.

h4(#dev-process-deploy). Deploy

At the end of the code review, the code reviewer merges the pull request, deploys the new functionality to production, and deletes the corresponding branch. We don't deploy to production on Fridays or before leaving for a vacation. To ensure that the master is always the deployed state, and to only close issues when they are actually deployed to production, merging the pull request should _not_ happen using the GitHub web UI, but on the production server:

1. Pull the feature branch into the master, e.g. @git -c "user.name=First Last" -c "user.email=name@example.com" pull --no-ff origin 111-ui@ (we have a single deployment user on our server, so we have to pass our name and email to the git pull command)
2. Switch proxy to staging, restart production app, test new functionality, switch proxy back to production if all is fine
3. Amend the merge commit: @git commit --amend@ to include a reference to the resolved issue with instructions on how to replicate the change in production (usually a link to the production system demonstrating the new functionality), e.g. @Resolves #238. See http://lobid.org/nwbib/search?q=Neuehrenfeld@
4. Push the changes to master: @git push origin master@, and delete the feature branch, e.g. @git push origin :111-ui@
