...
1. To avoid interruption of the service during deploy, configure the proxy to point to the staging server while updating the production server. In order to so, log in to the proxy and edit the Apache vhost file, e.g. @/etc/apache2/vhosts.d/beta.lobid.org.conf@. In this file, replace the port for @ProxyPass@ and @ProxyPassReverse@ with the port for the staging environment. Save the file and restart the Apache server: @rcapache2 restart-graceful@
...
3. Switch proxy to staging, restart production app, test new functionality, switch proxy back to production if all is fine
...
6. If you accomplished step 1, make sure you revert the port modification such that the proxy points to the production environment. Again, edit the Apache vhost file and replace the port for @ProxyPass@ and @ProxyPassReverse@ with the port for the production environment. Save the file and restart the Apache server: @rcapache2 restart-graceful@
